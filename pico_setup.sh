...
OPENOCD_DEPS="gdb-multiarch automake autoconf build-essential texinfo libtool libftdi-dev libusb-1.0-0-dev pkg-config" #Added pkg-config to make compatible with libusb
VSCODE_DEPS="code"
UART_DEPS="minicom"
...
echo "Clearing for fresh install"
sudo rm -d -r $OUTDIR #Doing this because mkdir fails randomly if it already exists
...
if [[ "$SKIP_VSCODE" == 1 ]]; then
   ...
   # sudo apt install -y $VSCODE_DEPS
    sudo snap install $VSCODE_DEPS --classic #VS Code only available through classic snap
...
# Enable UART 
# Skipping UART install for Ubuntu
# if [[ "$SKIP_UART" == 1 ]]; then
